...
[過去ログのページ境界](COMMENTS-HISTORY.md)を追加。既存100成功要求のadditionalsは全て空だったため、when付き取得の成功根拠には使わない。固定OSSの合成実行は「現在の実通信」と区別する。R12で未実行と記録した外部コードは、その後R13で対象部分だけ偽HTTPにより実行した。
